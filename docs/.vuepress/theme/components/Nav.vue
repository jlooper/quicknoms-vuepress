<template>
  <nav
    class="nav pr-0 lg:p-5 md:p-3 sm:p-0 bg-orange-900 text-white flex items-center justify-between flex-wrap text-right pr-0 "
  >
  
    <router-link
      class="w-1/4 md:w-1/4 text-left text-2xl uppercase no-underline cursor-pointer block"
      :to="$withBase('/')"
    >QuickNoms
    </router-link>
    ...
    <div v-on:click="openNav()" class="lg:hidden">
      <button
        class="flex m-5 px-3 py-2 border rounded border-white hover:text-orange hover:border-orange"
      >
